      ...
      DEFAULT_CONFIG = {
        ...
        def initialize(options = {})
          ...
          @tracer = base_config[:tracer]
				  @resque_service = base_config[:resque_service]
					@tracer.enabled = base_config[:enabled]
					Datadog::Tracer.debug_logging = base_config[:debug]
					@tracer.configure(
					hostname: base_config[:trace_agent_hostname],
					            port: base_config[:trace_agent_port]
					)
